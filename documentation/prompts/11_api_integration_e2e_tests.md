...
- Write guest review → both reviews updated to `published=true`
...
export const TEST_SPOTTER = { email: 'guest@test.spotzy.com', password: process.env.TEST_SPOTTER_PASSWORD! };
...
### E2E test 1: `e2e/journeys/guest-books-spot.spec.ts`

```typescript
test('Guest finds and books a parking spot end-to-end', async ({ page }) => {
  // 1. Log in as test guest
...
### E2E test 3: `e2e/journeys/host-guest-chat.spec.ts`

```typescript
test('Host and guest can exchange messages', async ({ browser }) => {
  ...
  // Guest receives message without page refresh
  // Guest replies "Thanks!"
  // Host receives reply
});
```

### E2E test 4: `e2e/journeys/guest-cancels-booking.spec.ts`

```typescript
test('Guest cancels booking and receives correct refund', async ({ page }) => {
  // Login as guest
...
```typescript
test('Guest opens a dispute via AI chat', async ({ page }) => {
  // Login as guest
...
1. **2 test users** in Cognito:
   ...
   - `guest@test.spotzy.com` (role: SPOTTER, Stripe test payment method attached)
