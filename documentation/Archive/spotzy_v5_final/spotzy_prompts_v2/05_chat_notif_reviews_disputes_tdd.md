...
**Happy path:**
- Valid message from spotter to host → stored in DynamoDB, pushed to host's WebSocket connection, 201 returned
- Valid message from host to spotter → stored, pushed to spotter's connection
...
- Requester is spotter or host → 200
- Unrelated user → 403
...
**Template rendering:**
- `booking.created` event → SMS to host includes spot address, spotter name, dates, amount
- `booking.cancelled` event → SMS sent to BOTH host AND spotter (two SNS publishes)
...
**Happy path — spotter rating host:**
...
**Happy path — host rating spotter:**
...
- Spotter tries to rate using host sections (SAFETY_SECURITY etc.) → 400 `INVALID_SECTION_FOR_ROLE`
- Host tries to rate using spotter sections → 400
...
**Happy path:**
- Spotter or host sends message → stored, 201 returned
