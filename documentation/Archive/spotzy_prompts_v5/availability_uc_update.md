...
7. **Conflicts found**: System blocks the save. Displays a conflict list: "The following bookings fall in periods you are removing: [booking list with dates and Guest names]." Host cannot proceed until conflicts are resolved (bookings must be completed or cancelled first).
...
| Field | Detail |
|---|---|
| **Goal** | Guest finds listings that are actually available for their intended period |
| **Primary Actor** | Guest |
| **Precondition** | Guest is on the search screen; Mapbox map loaded |
| **Trigger** | Guest enters a destination (with or without dates) |

### Main Flow — Search without dates

1. Guest enters a destination but does not specify dates.
...
6. Guest can tap a listing to view its availability calendar before booking.

### Main Flow — Search with dates

1. Guest enters a destination and specifies `startTime` + `endTime`.
