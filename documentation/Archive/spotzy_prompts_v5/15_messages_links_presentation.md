...
export const handler = async (event) => {
  ...
  // 1. Fetch all bookings for this user (both as host and guest)
...
### A3 — user-public-get Lambda update (guest profile enrichment)

**Tests first — add to `__tests__/users/public-get.test.ts`:**
```typescript
test('guest profile includes completedBookings count', async () => {
  const result = await handler(mockEvent({ pathParameters: { id: 'guest-123' } }));
  expect(JSON.parse(result.body).completedBookings).toBe(7);
});

test('guest profile includes responseRate when >= 5 bookings', async () => {
  const result = await handler(mockEvent({ pathParameters: { id: 'guest-123' } }));
...
test('responseRate is null when < 5 completed bookings (insufficient data)', async () => {
  const result = await handler(mockEvent({ pathParameters: { id: 'new-guest' } }));
...
// Person links
test('guest-side card: host name is a link to host profile', () => {
  render(<BookingCard booking={mockBookingAsSpotter} role="guest" />);
...
test('host-side card: guest name is a link to guest profile', () => {
...
test('all three links present on COMPLETED booking', () => {
  render(<BookingCard booking={{ ...mockBooking, status: 'COMPLETED' }} role="guest" />);
...
{/* Person link — host shows guest, guest shows host */}
<Link
...
test('Host name on guest booking card links to host profile', async ({ page }) => {
  await loginAsSpotter(page);
  await page.goto('/dashboard/guest');
...
test('All three links present on COMPLETED booking card', async ({ page }) => {
  await loginAsSpotter(page);
  await page.goto('/dashboard/guest?tab=past');
...
test('Guest presentation page shows response rate', async ({ page }) => {
  await loginAsHost(page);
