...
3. Guest-to-Host upgrade flow — Become a Host interstitial (UC-H19)
...
test('spotterId !== hostId → proceeds normally', async () => {
  ...
  const event = mockEvent({
    body: { listingId: listing.listingId, startTime: tomorrow(), endTime: tomorrowPlus2h() },
    requestContext: { authorizer: { claims: { sub: 'guest-xyz' } } },
  });
...
test('logged in, Guest only (no Stripe, no listings) → returns "/become-host"', () => {
...
```typescript
test('Guest clicking "List your spot" sees Become a Host screen', async ({ page }) => {
  await loginAsSpotter(page); // guest with no listings, no Stripe
