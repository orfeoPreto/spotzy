...
test('for guest: listings array is empty or absent', async () => { ... });
...
test('My bookings card shows count and links to /dashboard/guest', () => { ... });
...
test('guest profile does not show listings section', () => { ... });
...
test('guest name shown as hyperlink on host booking card', () => {
...
**Implementation:** Update `components/BookingCard.tsx` to render host/guest name as `<Link href={/users/{id}}>` wherever booking cards appear.
...
test('listings load immediately without Guest entering destination', async () => {
  render(<SearchPage />);
...
test('Navigation bar visible on all pages after login', async ({ page }) => {
  ...
  for (const path of ['/search', '/dashboard/guest', '/profile']) {
...
test('Profile page shows role badges and summary cards', async ({ page }) => {
  ...
  await expect(page.locator('[data-testid="guest-badge"]')).not.toBeVisible();
